...
function open_win(num){
 ...
 window.open('cert_copy.jsp?id='+num,"학력사항","width=1000,height=800,left=800,top=300");
...
<tr height = '20' style = "background: #D9E5FF;">
	<th width = '130'>발행번호</th>
	<th width = '130'>사원번호</th>
	<th width = '130'>성명</th>
	<th width = '130'>증명서종류</th>
	<th width = '130'>용도</th>
	<th width = '130'>발행일</th>
	<th width = '50'>인쇄</th></tr>

	<%
	for(CertEntity ce : (ArrayList<CertEntity>)list_cert) {
%>	
	<tr align = 'center'>
	<td><%=ce.getCert_num()%></td>
	<td><%=ce.getEmployee_num()%></td>
	<td><%=ce.getCert_employee_name()%></td>
	<td><%=ce.getCert_type()%></td>
	<td><%=ce.getCert_use()%></td>
	<td><%=ce.getCert_date()%></td>
	<td onclick="open_win(<%=ce.getEmployee_id()%>)"><a href = "">인쇄</a></td>
